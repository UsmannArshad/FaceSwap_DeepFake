...
frontal_face_predictor = dlib.shape_predictor("D:\DeepFake_Practice\FaceSwap\DataSet\shape_predictor_68_face_landmarks.dat")
webcam_video_stream=cv2.VideoCapture("D:\DeepFake_Practice\FaceSwap\image/imuvideo.mp4")
fourcc = cv2.VideoWriter_fourcc(*'DIVX')
frames_per_second = 20
frame_width = int(webcam_video_stream.get(3))
frame_height = int(webcam_video_stream.get(4))   
size = (frame_width, frame_height)
out = cv2.VideoWriter('output.avi',fourcc, frames_per_second, size)
#read the source face image and convert it to grayscale
source_image = cv2.imread("D:\DeepFake_Practice\FaceSwap\image/joker.jpg")
...
while (webcam_video_stream.isOpened):
    ...
    if destination_image is not None:
        ...
        if(len(destination_faces)==1):
            ...
            out.write(seamlesscloned_face)
        else:
            cv2.imshow("14: seamlesscloned_face", destination_image)  
            out.write(seamlesscloned_face)
